...
class deeplab_v3(nn.Module):
    def __init__(self,input_shape=(720,1280),backbone="xception",num_classes=3,OS=16):
        ...
        if self.backbone=="xception":
            ...
            self.in_channel1 = 2048
            self.in_channel2 = 2048
            ...
            self.in_channel1 = 320
            self.in_channel2 = 320
        self.pos1 = nn.Sequential(*[
                    nn.AvgPool2d((input_shape[0]//OS,input_shape[1]//OS)),
                    nn.Conv2d(self.in_channel1,256,1,bias=False),
                    # nn.BatchNorm2d(256),
                    nn.ReLU(),
                   nn.UpsamplingBilinear2d(size=(input_shape[0]//OS,input_shape[1]//OS)), ])
        
        self.s = nn.Sequential(*[
                        nn.Conv2d(self.in_channel2,256,1,bias=False),
                        nn.BatchNorm2d(256),
                        nn.ReLU()])
        self.in_channel = 256
        if self.backbone=="xception":
        ...
        in_channel = 256
        if self.backbone=="xception": 
            self.up1 = nn.UpsamplingBilinear2d(size = (input_shape[0]//4,input_shape[1]//4))
    ...
    def trainer(self,dataset,model=None,epoches=5000):
...
def model_framework():
    ...
    model = deeplab_v3()
    summary(model,torch.zeros(1,3,720,1280))     
